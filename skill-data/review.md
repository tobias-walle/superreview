...
{ url: string; reviewId: string; title: string; snapshotId: string; files: number }
...
The output includes `url`, `reviewId`, `title`, `snapshotId`, and `files`. Default comparisons resume the latest active review for the branch and worktree target. Use `--new --name "Auth review"` only when the user requests a separate review. Use `--review <id> main...HEAD` to attach a new comparison to a specified review.

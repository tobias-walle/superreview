...
3. Run `npm login`, then publish the initial `@tobiw/superreview` package with `cd dist-cli && npm publish --access public`.
4. Tag the same commit with `v0.2.1` and push the tag.
5. Configure `release.yml` as the trusted publisher for `@tobiw/superreview`. Bind it to the `npm-production` GitHub environment and allow `npm publish`.
